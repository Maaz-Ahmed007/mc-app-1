...
interface PrintableReportExpenseProps {
    data: TableData
    month: Date
    sectionId: string
}

export const PrintableReportExpense: React.FC<PrintableReportExpenseProps> = ({ 
    data, 
    month,
    sectionId
}) => {
        ...
        <div className="print-content p-8 max-w-7xl mx-auto bg-white">
            ...
            <div className="header-content">
                ...
                <p className="text-2xl font-semibold text-gray-700 mb-2">
                    Section: {companyInfo.sectionName} | Expense Final List - {format(month, 'MMMM yyyy')}
                </p>
                ...
                <table>
                    ...
                    <tbody>
                        {data.data.map((expense, index) => (
                            <tr key={index}>
                                <td>{expense.name}</td>
                                <td className="text-right">{currencyFormat.format(expense.amount)} DR</td>
                            </tr>
                        ))}
                        <tr className="total-row">
                            <td className="text-right">Final Balance:</td>
                            <td className="text-right">{currencyFormat.format(data.total)} DR</td>
                        </tr>
                    </tbody>
                </table>
            </div>
